...
void hasho2()
{
    LL n=ss.size(),m,i,j,a,b,c,x=0,y=0,z;
...
//    FOR(i,0,n-1)
//        cout<<ar[i].F<<' '<<br[i].F<<NL;
//    cout<<NL;
//    FOR(i,0,n-1)
//        cout<<ar[i].S<<' '<<br[i].S<<NL;
//    cout<<NL;

    c=a=b=0;
    FOR(i,0,min(n,m)-1)
    {
        if(ar[i].F==br[i].F&&ar[i].S==br[i].S)
        {
            ///if(i==7)cout<<i<<' '<<ar[i].F<<' '<<ar[i].S<<NL;
            j=i+1;a=b=1;
            while(j+i<n)
            {
                d=sub_hasho(ar,j,j+i);
                ///if(i==7)cout<<d.F<<' '<<d.S<<NL;
                if(d==ar[i]) a++;
                else break;
                j+=(i+1);
            }
