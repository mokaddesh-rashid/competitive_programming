...
void init(LL idx, LL b, LL e )
{
    ...
    cnd.maxx = max( cnd.maxx, cnd.sum );
    cnd.maxx = max( cnd.maxx, tree[lnd].smrt + tree[rnd].sum );
    cnd.smrt = max( cnd.sum, tree[rnd].sum );
    cnd.smrt = max( cnd.smrt, tree[rnd].smrt );
