...
int main()
{
   ...
   /// inf;
    ...
    FOR(tt,1,tc)
    {
        int K = 0, x, y, w, cc, Cost, n;
        sfff(n, cc, Cost);////cin >> n >> m >> K;

        FOR(i,0,cc-1)
        {
            sfff(info[i].u, info[i].v, info[i].b);
            sf(info[i].c);
        }

        int st, en, mid, prin = -1;
        st = 0; en = 1e6;
        ...
        while(st <= en)
        {
            ...
            {
                Node t = info[i];
                if(t.b >= mid) AddEdge(t.u, t.v, t.c);
            }
            int val = directedMST( K );

            if(val != -1 && val <= Cost)
                st = mid + 1, prin = max(mid, prin);
            else en = mid - 1;

        }


        if(prin == -1) printf("Case %d: impossible\n", tt);
