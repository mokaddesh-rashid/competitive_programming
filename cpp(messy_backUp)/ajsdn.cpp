...
int finPa(int p)
{
    return (paa[p] == p) ? p : (paa[p] = finPa( paa[p] ));
}
    ...
    FOR(i,1,n-1)
    {
        if(in[ id[p] ].F != in[ id[p - 1] ].F)
        {
            FOR(j,p,n-1)
            {
                ...
                {
                    if(!vis[ in[ id[j] ].S.F ] || !vis[ in[ id[j] ].S.S ])
            ...
            FOR(j,p,q-1)
            {
                ...
                if(r1 != r2)
                {
                    cout << "merge" << ' ' << r1 << ' ' << r2 << endl;
                    paa[r2] = r1;
                    sz[r1] += sz[r2];
                }
            }
           /// if(p != q) cout << p << ' ' << q << endl << endl << endl;
            p = q;
        }
        ...
        cout << id[i] << ' ' << x << ' ' << y << ' ' << sz[r1] << endl;

        int v = sz[r1], va = popo[x];
        v -= va;
        prin[ id[i] ] = (LL) v * va;

        if(prin[ id[i] ] > mo) mo = prin[ id[i] ],cnt = 0;
        if(prin[ id[i] ] == mo) cnt++;
    }
