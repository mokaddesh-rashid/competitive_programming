...
vector<LL> mat[sz], con[sz], bap[sz], cost[sz];
...
void col_info(LL p,LL vp)
{
    if(dhukao==1)
    {
        ...
        if(a>=0) pachi[cen].PB(cost[p][a]);///parent_theke_amr_child_r_distance
        bap[p].PB(cen);
        cost[p].PB(dis[p]);
        child[cen].PB(dis[p]);
    }
    ...
    FOR(i,0,(LL)mat[p].size()-1)
    {
       ...
       dis[v]=dis[p]+1;
       col_info(v,p);
       sub[p]+=sub[v];///adding child`s_sub_tree
    }
}
...
int main()
{
    ...
    //inf;
    //outf;
    LL i,j,n,m,a,b,x,y,t,v,z,k,c=0;
...
//    FOR(i,1,n)
//    {
//        cout<<child[i].size()<<NL;
//        FOR(j,0,(LL)child[i].size()-1)
//            cout<<child[i][j]<<' ';
//        cout<<NL;
//        FOR(j,0,(LL)pachi[i].size()-1)
//            cout<<pachi[i][j]<<' ';
//        cout<<NL<<NL;
//    }
    FOR(i,1,n)
    {
        ...
        REV(j,(LL)bap[i].size()-1,0)
        {
            ...
            low=lower_bound (child[a].begin(), child[a].end(), v);
            ...
            if(a!=t_cen)
            {
                ...
                low=lower_bound (pachi[a].begin(), pachi[a].end(), v);
