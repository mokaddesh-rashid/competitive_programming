...
{
    if(row[x] == {-1, -1}) row[x] = {x, y};
    else if(lev[ low[x].F ][ row[x].S ] < lev[ x ][ y ]) row[x] = {x, y};
}
...
int main()
{
    ...
    FOR(tt,1,tc)
    {
        ...
        FOR(i,0,n-1)
            ...
            if(row[i] != {-1, -1}) prin = min(prin, check(row[i].F< row[i].S));
        }
