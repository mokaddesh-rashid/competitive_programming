...
void mergeIt(int lev, int b, int e, int l, int r)
{

    for(int p = b; b <= e || l <= r; p++)
    {
        if(b > e || tree[lev][l].F <= tree[lev][b].F)
        {
...
//        if(st < en) mergeIt(lev, st, mid, mid + 1, en);
//        else tree[lev][st] = ar[st];
    }
    return;
}
