...
void getPalinHashAndOccurance(int lev)
{
    scanf("%s", olel);
    s = olel;
    int n = s.size();
...
void solve()
{
    ...
    getPalinHashAndOccurance(0);
    suff = 2;
    getPalinHashAndOccurance(1);

    FOR(i,3,id)
    {
        if(tree[i].numOfTime[0] < tree[i].numOfTime[1]) ara[2]++;
        else if(tree[i].numOfTime[0] == tree[i].numOfTime[1]) ara[1]++;
        else ara[0]++;
    }

    FOR(i,0,2) pf("%d ",ara[i]);
    pf("\n");
